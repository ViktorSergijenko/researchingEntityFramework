...
namespace WindowsFormsApp1.Services
{
    class EmployeeService
    {
        ...
        public async Task<List<Employee>> GetList()
        {
            ...
            var employee = await DB.Employees.Include(x => x.Boss).ThenInclude(x => x.Employees).ToListAsync();
        ...
        /// </summary>
        /// <param name="employee">New or updated object</param>
        /// <returns>New or updated object</returns>
        public async Task<Employee> Save(Employee employee)
        {
            if (employee.Id == null || employee.Id.Equals(Guid.Empty))
            {
        ...
        public async Task Delete(Guid id)
        {
            ...
            var weaponToDelete = await DB.Employees.FirstOrDefaultAsync(x => x.Id == id);
            ...
            DB.Employees.Remove(weaponToDelete);
            // Saving changes in DB
            await DB.SaveChangesAsync();
        }
        #endregion BaseCrud
        ...
        /// <returns></returns>
        public async Task<List<Employee>> FilterMethod(string filterSorting)
        {
            // Getting our query, that we will filter
            var query = await DB.Employees.Include(x => x.Boss).ToListAsync();
            ...
            if (!String.IsNullOrEmpty(filterSorting))
            {
                ...
                query = query.Where(x => x.FullName.ToLower().Contains(filter)
                || x.PersCode.Contains(filter)).ToList();
            }
            else
            {
                ...
                query = query.OrderByDescending(x => x.FullName).ToList();
            }
            return query;
        }
